...
--13: Regresa a los asesores que ganan más de 1500 de suelod
SELECT * FROM ASESOR WHERE SUELDO > 10000;
/

--14: Regresa el valor catastral más alto de la tabla casa
SELECT MAX(VALORCATASTRAL) FROM CASA;
/

--15: Regresa el CURP de los dueños que nacieron en 1998
SELECT *
FROM DUEÑO
JOIN DUEÑO_CURP
ON(DUEÑO_CURP."IDDUEÑO" = DUEÑO."IDDUEÑO")
JOIN CURP
ON(DUEÑO_CURP.IDCURP = CURP.IDCURP)
...
----17: Regresa a los dueños que no tienen ningun correo registrado
SELECT *
FROM DUEÑO
LEFT JOIN DUEÑO_EMAIL
ON(DUEÑO_EMAIL."IDDUEÑO" = DUEÑO."IDDUEÑO")
...
--19: Selecciona los asesores tal que su apellido paterno inicie con a
